...
function getCurrentUser() { try { return JSON.parse(sessionStorage.getItem(SK.user) || 'null'); } catch { return null; } }
...
/**
 * 🛠️ DECONECTARE SECURIZATĂ (ZERO-LOSS LOGOUT)
 */
...
window.profSelectAvatar = function(av) {
  const user = getCurrentUser();
  if (user) { user.avatar = av; saveCurrentUser(user); if (typeof window.cloudPushData === 'function') window.cloudPushData(); buildProfilePage(true); authUpdateTopBar(user); }
  profCloseAvatarPicker();
};
...
function socRenderLeaderboard() {
  ...
  list.innerHTML = rankings.map((r, i) => {
    ...
    return `<div class="soc-user-card" onclick="viewUserProfile('${r.username}')"><div style="font-weight:700; width:20px;">${i+1}</div><div class="soc-post-avatar">${renderAvatarContent(r.avatar)}</div><div style="flex:1;"><div class="soc-post-author ${isElite ? 'elite-nickname-platinum' : ''}">@${r.username} ${vBadgeHtml}</div><div class="soc-post-date">${r.wr}% WR • ${r.total} bilete</div></div></div>`;
  }).join('');
}
