...
import com.rgdbet.app.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun applyHdEffect() {
        val colorMatrix = ColorMatrix().apply {
            // Boost saturation and contrast for HD feel
            val contrast = 1.25f
            val brightness = 15f
            set(floatArrayOf(
        ...
        binding.bgGrass.colorFilter = ColorMatrixColorFilter(colorMatrix)
    }

    private fun fetchRealLotCount() {
        val authManager = (application as RgdbetApplication).authManager
        authManager.observeUserCount { count ->
            binding.tvUserCount.text = "LOT: $count"
        }
    }
    ...
    private fun startAnimations() {
        ...
        binding.floodlightView.setTargetPosition(215f)

        ObjectAnimator.ofFloat(binding.tvGoRegister, "translationX", -5f, 5f).apply {
            duration = 2000
            repeatCount = ValueAnimator.INFINITE
            repeatMode = ValueAnimator.REVERSE
        ...
        val views = listOf(binding.logoWrapper, binding.titleUnderline, binding.glassCard, binding.tvGoRegister, binding.btnLogin)
        views.forEachIndexed { i, view ->
            ...
            view.animate()
                ...
                .translationY(0f)
                .setDuration(1200)
                .setStartDelay(250L * i)
                .setInterpolator(AccelerateDecelerateInterpolator())
                .start()
        }

        // Futuristic pulse for the new buttons
        listOf(binding.btnLogin, binding.tvGoRegister).forEach { button ->
            ObjectAnimator.ofFloat(button, View.ALPHA, 1f, 0.85f, 1f).apply {
                duration = 2000
                repeatCount = ValueAnimator.INFINITE
                repeatMode = ValueAnimator.REVERSE
                start()
    ...
    private fun doLogin() {
        ...
        var hasError = false
        if (email.isEmpty()) { 
            binding.tilEmail.error = "ID REQUIRED"
            hasError = true
        }
        if (pass.isEmpty()) { 
            binding.tilPassword.error = "KEY REQUIRED"
            hasError = true
        }
    ...
    private fun showError(msg: String) {
        binding.tvError.text = "> ERROR: $msg"
        binding.tvError.visibility = View.VISIBLE
        
        playWhistleEffect()

        ObjectAnimator.ofFloat(binding.glassCard, "translationX", 0f, 25f, -25f, 25f, -25f, 0f).apply {
            duration = 500
            start()
        }
    }
